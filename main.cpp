...
void updateUtilitiesIncrementalGreedy(int node,unordered_map<int,int> &H,unordered_map<int,int> &oj,unordered_map<int,int> &ej,unordered_map<int,unordered_set<int>> &OC,unordered_map<int,unordered_set<int>> &EC,unordered_map<int,int> &S,unordered_map<pair<int,int>,int,hash_pair> &T,unordered_map<int,int> &PU,unordered_map<int,int> &U){
     for(auto it:T){
        int source=it.first.first,destination=it.first.second;
        int newoj=oj[source];
        int newej=ej[destination];
        if(OC[source].find(node)!=OC[source].end()){
           newoj=1;
        }
        else if(EC[destination].find(node)!=EC[destination].end()){
           newej=1;
        }
...
void intialiseGreedyIncremental(unordered_map<int,int> &oj,unordered_map<int,int> &ej,unordered_map<int,int> &PU,unordered_map<int,int> &U,unordered_map<int,int> &S,unordered_map<int,unordered_set<int>> &OC,unordered_map<int,unordered_set<int>> &EC,unordered_map<int,int> &H,unordered_map<int,int> &Ho,unordered_map<pair<int,int>,int,hash_pair> &T){
     ...
     for(auto it:Ho){
        int so=it.first;
        H[so]++;
        updateUtilitiesIncrementalGreedy(so,H,oj,ej,OC,EC,S,T,PU,U);
     }  
}
...
int incremental_Greedy(int k,unordered_map<int,int> &Ho,unordered_map<int,int> &S,unordered_map<int,unordered_set<int>> &OC,unordered_map<int,unordered_set<int>> &EC,unordered_map<pair<int,int>,int,hash_pair> &T,double thresholdDistance){
     ...
     for(int i=0;i<k;i++){
        ...
        if(count==0){
          maxUtilityHub=PU;
            ...
            H[node]++;
            updateUtilitiesIncrementalGreedy(node,H,oj,ej,OC,EC,S,T,PU,U);
            continue;
        }
        else if(count>1){
               for(auto it:U){
        ...
        for(auto it:PU){
            ...
            if(maxUtilityHub.find(it.first)!=maxUtilityHub.end()){
    ...
    for(auto it:T){
        int source=it.first.first;
        int destination=it.first.second;
        if(H.find(source)!=H.end() and H.find(destination)!=H.end()){
           totalTripCover++; 
        }
    }
